          ...
          <div className='flex justify-center w-full'>
            <div className='flex flex-col items-center gap-8'>
              <div className="flex flex-col items-center justify-center rounded-xl sm:rounded-3xl gap-2">
                <Image alt={product.name} className="rounded-3xl" height={500} src={selectedProduct.image_url} width={500} />
                <Typography className="text-alta-gray-500 max-w-80 text-center" variant='body-md'>{selectedProduct.name}</Typography>
                <Typography className="font-semibold text-alta-gray-500" variant='body-md'>${selectedProduct.price.toFixed(0)}</Typography>
              </div>
              <Button className='text-xl bg-alta-gray-900 rounded-3xl p-4 text-alta-white w-full transition ease-linear hover:bg-alta-700' variant='primary'>
                Add to cart
              </Button>
            </div>
          </div>
